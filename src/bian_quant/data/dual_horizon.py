...
def _is_partializable_funding_tail_coverage(
    ...
    """
    blocking_codes = {
        finding.code for finding in report.findings if finding.severity == QualitySeverity.BLOCKING
    }
    return (
    ...
    *,
    ...
    for index in range(len(plan)):
        ...
        if result is not None:
            ...
            if (
                ...
                partial_exclusions.append(_partial_exclusion(source, error_code=failure.error_code))
            else:
    ...
    if not blocking and ohlcv_frames and funding_frames and metrics_frames:
        ...
        if config.universe_policy is not None:
            popular_build = build_popular_universe_artifacts(
            ...
            blocked_periods.extend(shortage["identity_key"] for shortage in popular_build.shortages)
            blocked_periods = sorted(set(blocked_periods))
            blocking = bool(blocked_periods)
    ...
    affected_selection_days = sum(
        ...
        if has_funding_days_shortage(artifact, partial_assets)
    )
    partial_impact = {
